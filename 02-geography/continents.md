...
- **Pending fact to place:** traders from the "American supercontinent" (a working map-generation label — its relationship to the numbered continents above isn't reconciled yet) typically meet Lidia/Réselia traders in East African ports, which is part of why a port like Ouspré is valuable. This needs squaring with the continent table once continents 4 and 5 are pinned down, since "America" as used during map-building doesn't obviously map onto either of the small unnamed continents by size
